...
def promover_anno():
    ...
    for prestamo in prestamos:
        ListaNegra.objects.create(
            fecha=prestamo.fecha, estudiante=prestamo.estudiante, libro=prestamo.libro
        )
...
def logout_view(request):
    """Logout via POST and redirect back to the page the user came from.

    Falls back to the index if no referer is present.
    """
    logout(request)
